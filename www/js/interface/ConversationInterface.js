...
var ConversationInterface = (function () {
    function ConversationInterface() {
    }
    ConversationInterface.initialize = function () {
        ...
        $("#conversation-send-message-button").on("click", function () {
            if ($("#conversation-message-input").val().length > 160) {
                ...
                Api.sendSms(Settings.getUsername(), Settings.getPassword(), Settings.getLocalPhoneNumber(), Conversations.getConversations()[Conversations.getActiveConversationIndex()].getRemotePhoneNumber(), $("#conversation-message-input").val(), function (successful, err) {
                    if (successful) {
                        ConversationsInterface.displayConversations(function () {
                            ConversationInterface.displayConversation(Conversations.getConversations()[Conversations.getActiveConversationIndex()]);
                        });
                    }
                    else {
    ...
    ConversationInterface.displayConversation = function (conversation) {
