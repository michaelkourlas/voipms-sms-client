...
var SettingsInterface = (function () {
    function SettingsInterface() {
    }
    SettingsInterface.initialize = function () {
        var body = $("body");
            ...
            Api.getLocalPhoneNumbers(Settings.getUsername(), Settings.getPassword(), function (phoneNumbers, err) {
                if (err === null) {
                    if (phoneNumbers.length === 0) {
                        MainInterface.showToastNotification("No phone numbers available. Is SMS enabled on the phone" + "number you wish to use?");
                    }
                    else {
        ...
        saveButton.on("click", function () {
            if (isNaN(historyInput.val()) || parseInt(historyInput.val()) <= 0 || historyInput.val() > 90) {
                MainInterface.showToastNotification("Number of days of SMS history to retrieve must be an integer greater" + "than 0 and less than or equal to 90.");
            }
