...
var NewConversationInterface = (function () {
    function NewConversationInterface() {
    }
    NewConversationInterface.initialize = function () {
        ...
        $("#new-conversation-contacts-button").on("click", function () {
            window.plugins.ContactChooser.chooseContact(function (contactInfo) {
                if (contactInfo.phoneNumber === "") {
                    ...
                    var filteredPhoneNumber = contactInfo.phoneNumber.replace(/[^\d]/g, "").replace(/^.*(\d{10})$/, "$1");
        ...
        $("#new-conversation-send-button").on("click", function () {
            ...
            if (filteredPhoneNumber.length !== 10) {
                ...
                Api.sendSms(Settings.getUsername(), Settings.getPassword(), Settings.getLocalPhoneNumber(), filteredPhoneNumber, $("#new-conversation-message-textarea").val(), function (successful, err) {
                    if (successful) {
                        history.back();
                    }
                    else {
    ...
    return NewConversationInterface;
})();
//# sourceMappingURL=NewConversationInterface.js.map