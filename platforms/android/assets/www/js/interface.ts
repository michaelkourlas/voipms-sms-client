        ...
        Conversations.refreshConversations(function(err) {
            ...
            if (err === null || initial) {
                ...
                if (conversations != null) {
                    ...
                    var counter = 0;
                    async.eachSeries(conversations, function(conversation, callback: () => void) {
                        Phone.getContact(conversation.getRemotePhoneNumber(), function(contact) {
                            ...
                            var listviewItemLink = $("<a id=\"conversations-listview-item-" + counter + "\" " +
                                "name=\"conversations-listview-item\" href=\"#\">");
                                ...
                                if (contact.photos !== null && contact.photos.length > 0) {
                                    $.get(contact.photos[0].value).done(function() {
                                        ...
                                        if (contact.displayName != null) {
                                            ...
                                            listviewItemLink.append("<h2>" + conversation.getRemotePhoneNumber() +
                                                "</h2>");
                                        }
                                        continueFunction();
                                    });
            ...
            function final() {
                ...
                $("a[name=\"conversations-listview-item\"]").on("click", function() {
                    if ($(this).attr("id").substr(0, 28) === "conversations-listview-item-") {
                        ...
                        Interface.displayConversation(Conversations.getConversations()[
    ...
    static displayConversation(conversation: Conversation) {
        ...
        Phone.getContact(conversation.messages[0].getRemotePhoneNumber(), function(contact) {
            ...
            for (var i = 0; i < conversation.messages.length; i++) {
                ...
                listviewItem.append($("<p class=\"ui-li-aside label-datetime-outer\">" +
                    "<span class=\"label-datetime-inner\">" + date + "</span></p>"));
