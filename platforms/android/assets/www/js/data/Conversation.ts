...
class Conversation {
     ...
     */
    public static createConversation(savedObject: any): Conversation {
        try {
    ...
    public addMessage(message: Message) : void {
        if (this.messages.length == 0 || message.date.isSame(this.messages[this.messages.length - 1].date) ||
            message.date.isAfter(this.messages[this.messages.length - 1].date)) {
     ...
     */
    public equals(conversation: Conversation): boolean {
     ...
     */
    public containsUnreadMessages(): boolean {
        for (var i = 0; i < this.messages.length; i++) {
